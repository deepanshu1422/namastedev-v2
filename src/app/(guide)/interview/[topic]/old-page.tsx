...
import { interviewQuestionsOld as interviewQuestions } from "@/util/globals";
...
export default function TopicQuestions({
  ...
  const topicContent =
    interviewQuestions[topic.toLowerCase()] || ({} as TopicContent);
  ...
  const renderContent = (content: Question[], section: string) => (
    <div className="space-y-6">
      {content.map((item, index) => (
        <div
          key={index}
          className="bg-footer/50 rounded-lg overflow-hidden shadow-md"
        >
          <div
            className="p-4 cursor-pointer transition-colors hover:bg-opacity-80"
            onClick={() => toggleQuestion(section, index)}
          >
            <h3 className="text-base sm:text-lg font-medium leading-tight mb-2 whitespace-pre-wrap">
              {item.question}
            </h3>
          ...
          {[
            {
              ...
              content: topicContent.interviewQuestions,
            },
            {
              title: "Take Home Projects",
              content: topicContent.takeHomeProjects,
            },
            {
              title: "Questions for Big Companies",
              content: topicContent.bigCompanyQuestions,
            },
                ...
                <CardTitle className="flex justify-between items-center text-xl">
                  ...
                  <ChevronDown
                    className={`h-6 w-6 transition-transform ${
                      openSections.includes(title) ? "transform rotate-180" : ""
                    }`}
                  />
                </CardTitle>
              </CardHeader>
