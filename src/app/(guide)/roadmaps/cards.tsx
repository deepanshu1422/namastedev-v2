...
// import { Chip } from "@/components/ui/chip"; // Add this import

export default function Cards({ data }: { data: typeof roadmapsData }) {
  return data.length > 0 ? (
    <>
      {/* <div className="flex flex-wrap gap-3 justify-center max-w-[84rem] mx-auto p-5">
        {data.map(({ name, slug }) => (
          <Link key={slug} href={`/roadmaps/${slug}`}>
            <div className="p-3 border border-white rounded-full text-secondary-foreground text-sm font-medium">
              {name}
            </div>
          </Link>
        ))}
