...
export default function Main({ title, slug, courseId, courseImage, modulesCollection }: {
    title: string;
    slug: string
    courseId: string,
    courseImage: {
    ...
    function nextVideo() {
        ...
        if (vidIndex.chapterIndex === modulesCollection.items[vidIndex.modIndex].chaptersCollection.total - 1 && vidIndex.modIndex !== modulesCollection.total - 1) {
            setVidIndex({
                modIndex: vidIndex.modIndex + 1,
                chapterIndex: 0
            })
        }
        if (vidIndex.chapterIndex !== modulesCollection.items[vidIndex.modIndex].chaptersCollection.total - 1) {
            setVidIndex({ ...vidIndex, chapterIndex: vidIndex.chapterIndex += 1 })
        }
    }
            ...
            <div className="hidden border-r bg-muted/40 md:block">
                <div className="flex h-full max-h-screen flex-col gap-2 sticky top-0">
                    <div className="sticky z-10 top-0 flex items-center border-b bg-bg">
                        <Link href="/dashboard" className="h-14 bg-muted/40 px-4 py-2 lg:h-[60px] lg:px-6 w-full flex items-center gap-2 font-semibold">
                        ...
                        <nav className="grid items-start px-2 text-sm font-medium lg:px-4">
                            {modulesCollection.items.map(({ title, chaptersCollection }, modIndex) => (
                                <div key={modIndex} className="flex flex-col gap-2">
                                    ...
                                    {chaptersCollection.items.map(({ title, public: free }, chapterIndex) => (
                                        <div key={chapterIndex} className="flex flex-col gap-2">
                                            ...
                                            >
                                                ...
                                                <span className="hidden text-xs text-left md:block">
                                                    {title} {free && <Badge className='px-1 text-xs text-white w-fit bg-prime/40 hover:bg-prime/60 rounded'>Free</Badge>}
                                                </span>
                                            </button>
                                        </div>
                                    ))}
                ...
                <div className="flex-1 flex flex-col gap-1 items-center p-2 md:pt-10 lg:pt-6 max-md:pt-20 pb-10">
                    ...
                    {modulesCollection.items[vidIndex.modIndex].chaptersCollection.items[vidIndex.chapterIndex].public || session?.user?.courseId?.includes(courseId) ? <><span className="text-xl md:text-2xl lg:text-3xl p-2 rounded-md bg-prime/30 mb-3">{modulesCollection.items[vidIndex.modIndex].chaptersCollection.items[vidIndex.chapterIndex].title}</span>
                        ...
                        <div className='flex justify-between w-full max-w-[800px]'>
                            ...
                            <Button onClick={() => nextVideo()} disabled={vidIndex.chapterIndex ===
                        ...
                        </div></> : <div className='flex flex-col gap-3 p-2 max-w-[700px]'>
                        <span className="text-xl md:text-2xl lg:text-3xl p-2 py-1 rounded-md bg-prime/30">Unlock the complete course</span>
