    ...
    <div className="grid min-h-screen w-full md:grid-cols-[60px_1fr] lg:grid-cols-[280px_1fr]">
      <div className="hidden border-r bg-muted/40 md:block">
        <div className="flex h-full max-h-screen flex-col gap-2 sticky top-0">
          ...
          <Accordion type="single" collapsible className="w-full shrink overflow-auto horizontal-scroll h-full">
            {/* <div className=""> */}
              <nav className="grid items-start px-2 text-sm font-medium lg:px-4">
                ...
                {navBar.map(({ href, icon, selected, title, recent }, i) =>
                    ...
                    <AccordionItem value={`item-${i+1}`}>
                      ...
                      <AccordionContent className="flex flex-col gap-1 px-3 max-lg:py-2 lg:py-1 ml-5 border-l-[1.5px] border-zinc-600">
                        {recent?.map(({ link, title }, i) => (
                          <Link className="rounded-lg px-2 py-1.5 text-xs text-muted-foreground transition-all hover:text-prime hover:bg-second/20" href={link}>{title}</Link>
                        ))}
                      </AccordionContent>
                    </AccordionItem>
                  ) : (
