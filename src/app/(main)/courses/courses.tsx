    ...
    return (<>
        <section className='grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 max-w-[85rem] gap-7 mx-auto max-phone:px-6 phone:px-10 max-tab:py-5 tab:pb-14'>
            {state ? courses.courseCollection.items.filter((e) => e.title.toLowerCase().includes(state.toLowerCase())).map(({ courseImage, tags, slug, title }, i) => (
                <Link key={i} href={`/courses/${slug}`} className='rounded bg-second/40 flex flex-col group transition-all duration-200 hover:bg-second/60 hover:shadow-xl shadow-black overflow-hidden sm:mb-3 max-w-xs w-[90%] sm:max-w-xs mx-auto'>
