    ...
    <div className={`w-full grid relative overflow-hidden`}>
      <div className="tab:p-[4.5rem_5.5rem_2.75rem] max-tab:pt-[4rem] max-tab:pb-[2rem] m-auto max-w-lg md:max-w-[75rem] grid gap-5 md:gap-7 place-items-center text-white text-center">
        ...
        <section className="md:py-2 flex gap-4">
          <Link
            target="_blank"
            href={`https://api.whatsapp.com/send?text=${title} - 30DC Blog https://30dayscoding.com/blog/${slug}`}
          ...
          <Link
            href={`mailto:?subject=Do check our latest blog on '${title}'&body=Check out new blog on ${title} via visiting - https://30dayscoding/blog/${slug}.`}
          ...
          <Link
            href={`https://twitter.com/intent/tweet?text=Read the blog from 30dayscoding on ${title} - ${BASE_URL}/blog/${slug}`}
            title={"Share via X"}
            target="_blank"
          >
            <svg
      ...
      <div className="grid gap-2">
        ...
        <Link
          ...
          target="_blank"
          href={`https://30dayscoding.com/bundle/complete-package-all-course-bundle?utm_source=${"https://30dayscoding.com/blog/" + slug}&utm_medium=blog&utm_campaign=banner`}
        >
          <Alert className="border-prime/80 bg-gradient-to-b from-prime/60 to-second/60 shadow-lg text-left max-w-2xl gap-1.5 flex flex-col">
            <AlertTitle className="font-bold md:text-lg">🎉 Exclusive Deal Alert!  17 premium courses for just ₹999.</AlertTitle>
            <AlertDescription className="max-md:text-xs">
              Unlock your potential with our expert-led courses—master skills,
              earn certificates, and transform your career today! <span className="text-primary underline">here.</span>
            </AlertDescription>
          </Alert>
        </Link>
      </div>
